/**
 * This file holds the functionality for the Village fragment.
...
class VillageFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        favoriteButton.setOnClickListener {
            val villageImage = ResidenceImage(
                id = R.drawable.village,
                imageUrl = "https://example.com/village.jpg",
                title = "Village Building"
            )

            // Check if the building is already a favorite
            if (sharedViewModel.isResidenceFav(villageImage)) {
                sharedViewModel.removeResidenceFav(villageImage)
                Toast.makeText(requireContext(), "Removed from favorites", Toast.LENGTH_SHORT).show()
            } else {
                sharedViewModel.addResidenceFav(villageImage)
